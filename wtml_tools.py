...
def format_wwt_url(
    ...
    **kwargs,
    ...
    url_rot = (
        rot + 180
    )  # https://wwt-api-client.readthedocs.io/en/latest/endpoints/legacy/show-image.html#endpoint-showimage
    if url_rot > 360:
        url_rot = url_rot - 360
    options = (
        reverse_parity,
...
def create_wtml(
    ...
    imageset = create_imageset_dict(
        name, **wtml_params, height=height, width=width, url=url
    )
    wwt_url = wwt_url_from_header(
        header, name, url, thumb_url=thumbnail_url or image_path, nudge=nudge
    )
    ...
    el_thumbnail_url.text = thumbnail_url or image_path
...
def add_description_credits_to_wtml(
    wtml_file,
    description="description",
    credits="credits",
    credits_url="credits_url",
